...
def _is_quota_error(exc: Exception) -> bool:
    msg = str(exc).lower()
    return any(k in msg for k in ("quota", "rate limit", "429", "resource_exhausted", "resource exhausted"))
...
class GeminiProvider(BaseAIProvider):
    ...
    def generate_markdown(self, text: str) -> str:
        ...
        try:
            ...
            raise ProviderError(f"Gemini text generation failed: {exc}") from exc
    ...
    def generate_markdown_from_image(self, image_bytes: bytes, mime_type: str) -> str:
        ...
        try:
            ...
            if _is_quota_error(exc):
                raise QuotaExceededError(str(exc)) from exc
            raise ProviderError(f"Gemini vision generation failed: {exc}") from exc
    ...
    def generate_markdown_from_images(self, images_bytes: List[bytes]) -> str:
        ...
        try:
            ...
            if _is_quota_error(exc):
                raise QuotaExceededError(str(exc)) from exc
            raise ProviderError(f"Gemini multi-image vision generation failed: {exc}") from exc
